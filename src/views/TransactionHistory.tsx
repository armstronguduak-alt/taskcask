...
export const TransactionHistory: React.FC = () => {
  ...
  const filteredTx = transactions.filter((tx) => {
    ...
    if (filter === 'Earnings') return tx.type === 'WatchReward' || tx.type === 'TaskReward' || tx.type === 'DailyReward';
    ...
    <div className="flex-grow pb-32">
      ...
      <nav className="sticky top-0 w-full z-30 bg-[#f8f9ff]/90 dark:bg-[#09090b]/90 backdrop-blur-md border-b border-gray-100 dark:border-zinc-900">
        <div className="flex items-center gap-stack-md px-container-padding py-4 w-full">
          <button onClick={() => setTab('Dashboard')} className="ripple-active p-1 text-primary">
            <span className="material-symbols-outlined">arrow_back</span>
          </button>
          <h1 className="font-bold text-lg text-primary dark:text-[#62df7d]">Statement logs</h1>
        </div>
      </nav>
              ...
              return (
                <div key={tx.id} className="flex justify-between items-center border-b border-gray-50 dark:border-zinc-800/50 pb-3 last:border-b-0 last:pb-0">
                  ...
                  <div className="text-right">
                    <p className={`font-extrabold text-xs ${isIncome ? 'text-primary dark:text-[#62df7d]' : 'text-orange-500 dark:text-orange-400'}`}>
